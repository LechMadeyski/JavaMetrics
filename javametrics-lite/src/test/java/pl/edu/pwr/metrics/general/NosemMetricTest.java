...
class NosemMetricTest {
  private static String metricName = "NOSE_M";
  private static final NosemMetric metric = new NosemMetric();
  ...
  void combinedTest() {
    ...
    var cu = StaticJavaParser.parse(fixture);
    var got = metric.compute(cu);
    var want = List.of(new Metric<>(metricName,1, null), new Metric<>(metricName,1, null));

    Assertions.assertThat(got).usingRecursiveComparison().ignoringFields("details").isEqualTo(want);
  }

  @Test
  void methodAndFieldTest() {
    var fixture = """
      class Fixture {
         var z = switch("TEST") {
          case "TEST":
